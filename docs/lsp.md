...
* **command_parameters** (optional): The command parameters. Parameters can be set from the **command** also (although default configuration will separate binary name from CLI parameters, and if binary name contains spaces it must always be separated from its parameters. *command_parameters* can be used to set the parameters to the original command (replacing any parameters set by default). The LSP configuration can be overwritten with the user `lspclient.json` file, located in the user configuration. For example: a user trying to set some specific command line arguments (overriding the default ones) to clang would need to do something like: `{"name": "clangd","command_parameters": "-log=error --background-index --completion-style=bundled -j $NPROC --background-index-priority=background --use-dirty-headers"}`
